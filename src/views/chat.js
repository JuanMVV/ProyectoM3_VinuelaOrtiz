...
const EASTER_EGGS = {
  ...
  42: {
    text: "🌌 La respuesta al sentido de la vida, el universo y todo lo demás.",
    meta: "🥚 Easter egg",
  },
  gracias: {
    text: "¡De nada! 😊 Recordá: la ciencia nunca termina, solo encuentra nuevas preguntas.",
    meta: "",
  },
};
...
async function retryOnceAfter429(error, payload) {
  ...
  appendMessage(
    "assistant",
    aiText || "No recibí texto en la respuesta.",
    truncated ? "⚠️ truncada" : "",
  );
  showStatus("hidden");
}
...
export function renderChat(characterKey) {
  const key = characterKey || "mikeWazowski";
...
function getThemeKey(name) {
  if (name.includes("Chavo")) return "chef";
